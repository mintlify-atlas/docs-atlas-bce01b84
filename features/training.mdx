...
<ParamField path="save_only_latest" type="bool" default="false">
...
<ParamField path="total_epoch" type="int" default="500">
...
<ParamField path="batch_size" type="int" default="8">
  Number of audio segments processed per training step. Larger batch sizes speed up training but require more VRAM. Reduce to `4` or `2` on GPUs with limited memory.
</ParamField>
...
<ParamField path="vocoder" type="str" default="HiFi-GAN">
  Vocoder used for audio synthesis during training. `HiFi-GAN` is the standard and only officially supported option in current Applio releases.
</ParamField>
