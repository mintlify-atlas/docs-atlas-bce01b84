...
description: "Fuse two trained RVC models into a single new voice at a configurable blend ratio using Applio's Voice Blender tool."
---

The Voice Blender tool lets you fuse two independently trained RVC `.pth` models into a single new model by linearly interpolating their weight tensors at a configurable ratio. The result is a model whose voice characteristics sit somewhere between the two source models — closer to Model A at a low ratio, closer to Model B at a high ratio, and an equal mix at 0.5. This is a quick way to create hybrid voices, smooth out weaknesses of one model with strengths of another, or explore the latent space between two similar voices without retraining from scratch.
...
<ParamField path="pth_path_1" type="str" required>
  Path to the first source `.pth` model file. At a ratio of `0.0`, the output is essentially this model; at `1.0`, only the second model's weights contribute.
</ParamField>

<ParamField path="pth_path_2" type="str" required>
  Path to the second source `.pth` model file. At a ratio of `1.0`, the output is essentially this model.
</ParamField>

<ParamField path="ratio" type="float" default="0.5">
  Blend ratio on a scale of **0.0 to 1.0**.
  - `0.0` → output is 100% Model 1
  - `0.5` → equal mix of both models
  - `1.0` → output is 100% Model 2

  The ratio is applied element-wise to every weight tensor. For the `emb_g.weight` (speaker embedding) tensor, if the two models have different numbers of speakers, only the minimum number of speaker rows are blended.
</ParamField>
